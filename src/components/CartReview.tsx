...
const MANUAL_VACIO: ProductoManual = {
  nombre: "",
  categoria: "",
  unidad: "",
  precio: "",
...
export function CartReview({
  ...
  function submitManual() {
    ...
    if (!manual.nombre.trim()) {
      setErrManual("El nombre es obligatorio.");
      return;
    }
    if (!manual.unidad.trim()) {
      setErrManual("La unidad es obligatoria.");
      return;
    }
    if (isNaN(precio) || precio <= 0) {
      setErrManual("Ingresa un precio válido.");
      return;
    }
    if (isNaN(cantidad) || cantidad <= 0) {
      setErrManual("Ingresa una cantidad válida.");
      return;
    }
    onAgregarManual(
      manual.nombre.trim(),
      manual.categoria.trim() || "Sin categoría",
      manual.unidad.trim(),
      precio,
      cantidad,
    );
                        ...
                        {editingNotaId === item.cartKey ? (
                          <div className="mt-2 flex gap-1.5 items-start">
                            ...
                            <div className="flex flex-col gap-1">
                              <button
                                onClick={() => confirmEditNota(item.cartKey)}
                          ...
                          <button
                            onClick={() =>
                              startEditNota(item.cartKey, item.nota ?? "")
                            }
                      ...
                      <div className="flex-1 min-w-0">
                        {editingPriceId === item.cartKey ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                startEditPrice(item.cartKey, item.precio)
                              }
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={manual.nombre}
                      onChange={(e) =>
                        setManual((p) => ({ ...p, nombre: e.target.value }))
                      }
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={manual.unidad}
                        onChange={(e) =>
                          setManual((p) => ({ ...p, unidad: e.target.value }))
                        }
                        className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]/20 focus:border-[#1a3a6b] transition"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={manual.precio}
                          onChange={(e) =>
                            setManual((p) => ({ ...p, precio: e.target.value }))
                          }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setManual((p) => ({ ...p, cantidad: e.target.value }))
                        }
                        className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]/20 focus:border-[#1a3a6b] transition"
                      />
                    </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-gray-100 text-gray-400 uppercase tracking-wide">
                        <th className="text-center pb-2 pr-2 font-semibold w-6">
                          #
                        </th>
                        <th className="text-left pb-2 pr-2 font-semibold">
                          Producto
                        </th>
                        <th className="text-center pb-2 pr-2 font-semibold">
                          Unidad
                        </th>
                        <th className="text-center pb-2 pr-2 font-semibold">
                          Cant.
                    ...
                    <tbody>
                      {cart.items.map((item, idx) => (
                        ...
                        >
                          <td className="py-1.5 pr-2 text-center text-gray-400">
                            {idx + 1}
                          </td>
                          ...
                          <td className="py-1.5 pr-2 text-center text-gray-700 font-semibold">
                            {item.cantidad}
                          </td>
