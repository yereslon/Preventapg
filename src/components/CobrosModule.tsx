...
interface CobroCardProps {
  ...
  onActualizar: (
    campos: Partial<
      Pick<CobroCliente, "nombre" | "efectivo" | "yape" | "comentario">
    >,
  ) => void;
  onEliminar: () => void;
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="bg-emerald-50 rounded-xl p-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="0.00"
              onChange={(e) =>
                onActualizar({ efectivo: parseFloat(e.target.value) || 0 })
              }
        ...
        <div className="bg-violet-50 rounded-xl p-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="0.00"
              onChange={(e) =>
                onActualizar({ yape: parseFloat(e.target.value) || 0 })
              }
      ...
      <div className="px-3 py-2.5 border-t border-gray-50 flex items-center gap-1">
        ...
        >
          ...
          {tienesFotos
            ? `${cobro.fotos.length} foto${cobro.fotos.length > 1 ? "s" : ""}`
            : "Foto"}
        </button>
        <button
    ...
    try {
      ...
      setFeedback(
        n === 0
          ...
          : `${n} pedido${n > 1 ? "s" : ""} importado${n > 1 ? "s" : ""}`,
      );
    } finally {
      setImportando(false);
    ...
    <section className="space-y-3">
      ...
      {cobros.length === 0 && (
        <div className="text-center py-10 text-gray-400">
          ...
          <p className="text-xs mt-1 text-gray-300">
            Toca &ldquo;+ Agregar cobro&rdquo; para empezar
          </p>
        </div>
      )}
      ...
      {cobros.map((cobro) => (
        <CobroCard
          ...
          onCambiarCliente={() =>
            setModalCliente({ modo: "editar", cobroId: cobro.id })
          }
        />
      ))}
        ...
        <div className="bg-gray-50 rounded-2xl border border-gray-100 p-4 space-y-2">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-extrabold text-emerald-700 tabular-nums">
              {formatSoles(totalEfectivo)}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-gray-400 text-xs">Total Yape / Plin</span>
            <span className="font-extrabold text-violet-700 tabular-nums">
              {formatSoles(totalYape)}
            </span>
          </div>
